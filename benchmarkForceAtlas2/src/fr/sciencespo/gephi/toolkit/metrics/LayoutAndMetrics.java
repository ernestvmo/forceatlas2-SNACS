...
public class LayoutAndMetrics {
    ...
    private void benchmark(File input_file, File output_dir, String algoSignature, Process process) {
        ...
        try {
            BufferedWriter br = new BufferedWriter(
                    new FileWriter(output_dir + File.separator + algoSignature + "_" + input_file.getName() + ".csv"));
            br.write(csvText);
            br.close();
        } catch (IOException ex) {
            ex.printStackTrace();
        ...
        System.out.println("#### Ending processing " + algoSignature + " - " + input_file.getName() + " in "
    ...
    private String buildReportRow(GraphModel graphModel, String algoSignature, int step) {
        ...
        String line = String.valueOf(step) + "," + String.valueOf(computeNeal(graphModel.getGraph()) + "," + time);
    ...
    public double computeNeal(Graph graph) {
        ...
        for (Edge e : edges) {
            Node sourceData = e.getSource();
            Node targetData = e.getTarget();
            double distance = Math.sqrt((sourceData.x() - targetData.x()) * (sourceData.x() - targetData.x())
                    + (sourceData.y() - targetData.y()) * (sourceData.y() - targetData.y()));
            sum_edges_distances += distance;
        }
        double sum_npairs_distances = 0;
        for (Node n1 : nodes) {
            for (Node n2 : nodes) {
                if (Integer.parseInt((String) n1.getId()) < Integer.parseInt((String) n2.getId())) {
                    double distance = Math
                            .sqrt((n1.x() - n2.x()) * (n1.x() - n2.x()) + (n1.y() - n2.y()) * (n1.y() - n2.y()));
    ...
    public void computeEdgeCrossings() {
         ...
         * if(doLineSegmentsIntersect(sourceData1.x(), sourceData1.y(), targetData1.x(),
         * targetData1.y(), sourceData2.x(), sourceData2.y(), targetData2.x(),
         * targetData2.y())){
         * c += 1;
         * }
         * }
    ...
    public boolean doLineSegmentsIntersect(double px, double py, double p2x, double p2y, double qx, double qy,
        ...
        if (((px == qx) && (py == qy)) || ((px == q2x) && (py == q2y)) || ((p2x == qx) && (p2y == qy))
                || ((p2x == q2x) && (p2y == q2y))) {
            return false;
        }
